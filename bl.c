...
unsigned char* BFishCrypt(unsigned char* iSecret, unsigned char* StrClear, unsigned long Lng) {
    BLOWFISH_KEY kStruct;
    unsigned char* StrCrypt;
    unsigned char _Clear[9], _Crypt[9];
    unsigned char __EOB[8] = {0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x08};
    unsigned long i,j,LnMalloc;

    key_schedule(iSecret, &kStruct, (unsigned int)max(strlen(iSecret), 8*(unsigned int)(strlen(iSecret)/8)));

    if (Lng % 8 == 0)
        LnMalloc = Lng + 8;
    else
        LnMalloc = (int)(8+8*((int)(Lng/8)));

    StrCrypt = (unsigned char*)malloc(sizeof(unsigned char)*LnMalloc + 1);
    if (!StrCrypt)
        return NULL;
    memset(StrCrypt, 0, LnMalloc + 1);

    memset(_Clear, 0, 9);
    memset(_Crypt, 0, 9);
    for(i = 0; i < Lng; i += 8) {
        memcpy(_Clear, StrClear+i, 8);

        if (i + 8 > Lng) {
            for (j = 0; j < (8 - (Lng-i)); j++)
                    _Clear[7-j] = (uchar)(8 - (Lng-i));
        }

        blowfish_encrypt(_Clear, _Crypt, &kStruct);
        memcpy(StrCrypt + i, _Crypt, 8);

        if (i + 8 == Lng) {
            memset(_Crypt, 0, 9);

            blowfish_encrypt(__EOB, _Crypt, &kStruct);
            memcpy(StrCrypt + i + 8, _Crypt, 8);
        }
    }
...
unsigned long BFishUncrypt(unsigned char* iSecret, unsigned char* StrCrypt, unsigned long Lng) {
    BLOWFISH_KEY kStruct;
    unsigned char _Clear[9], _Crypt[9];
    unsigned long i, Len;


    /** Is the Cipher Block look likes a Blowfish ? */
    if (Lng % 8 != 0)
        return 0;
    ...
    for(i = 0; i < Lng; i += 8) {
        memset(_Clear, 0, 9);
        memset(_Crypt, 0, 9);
        memcpy(_Crypt, StrCrypt+i, 8);

        blowfish_decrypt(_Crypt, _Clear, &kStruct);
        memcpy(StrCrypt + i, _Clear, 8);
    }

    if ((StrCrypt[Lng-1] != (uchar)1) && (StrCrypt[Lng-1] != (uchar)2) && (StrCrypt[Lng-1] != (uchar)3)
        && (StrCrypt[Lng-1] != (uchar)4) && (StrCrypt[Lng-1] != (uchar)5) && (StrCrypt[Lng-1] != (uchar)6)
        && (StrCrypt[Lng-1] != (uchar)7) && (StrCrypt[Lng-1] != (uchar)8))
        return 0;

    Len = Lng - StrCrypt[Lng-1];
    memset(StrCrypt + Lng - StrCrypt[Lng-1], 0, StrCrypt[Lng-1]);
